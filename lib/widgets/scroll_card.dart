  ...
  Future<void> _handleTap(BuildContext context,
    ...
    if (newCard.type == CardClass.tpru) {
      ...
      print('сколько теперь каунт потмоу что мы добавили1 ${Provider
        ...
        .of<GameDataProvider>(context, listen: false)
        .actCount == 0) {
      // SnackBarService.showSnackBar(context, 'Take a card from the deck', true);
      DialogWindow.show(context, 'Take a card from the deck', 'Notification');
    } else if (Provider
        .of<GameDataProvider>(context, listen: false)
        .actCount == 2) {
      // SnackBarService.showSnackBar(context, 'You have finished change your turn', true);

      DialogWindow.show(context, 'You have finished change your turn', 'Notification');
    } else if (Provider
        .of<GameDataProvider>(context, listen: false)
        .actCount >= 3
    && Provider.of<GameDataProvider>(context, listen: false).actCount <10) {
      if (Provider
